...
export abstract class Enum {
    protected constructor(public readonly number: number, public readonly name: string) {
        // empty
    }
    toString(): string {
        return this.name;
...
export function enumValues(Type: any): Enum[] {
    const values: Enum[] = [];
...
export function enumDesc(id: number, Type: any): TypeDesc {
    return new TypeDesc(id, new EnumTypeHandler(Type.VALUES));
}
...
export abstract class Client {
    proxy<P>(contractId: ContractId<P, any>, interceptor = DIRECT_INTERCEPTOR): P {
    ...
    ws.onopen = () => {
        try {
            const session = Session.create(sessionFactory, {
                write: packet => ws.send(writeTo(packetSerializer, packet)),
                closed: () => ws.close()
            });
            ws.onmessage = event => {
                try {
                    Session.received(session, readFrom(packetSerializer, event.data));
                } catch (e) {
