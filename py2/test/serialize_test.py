...
class Test(unittest.TestCase):
    def genericCheck(self, value, b, write, read):  # type: (Any, List[int], str, str) -> None
        io = BytesIO()
        writer = createWriter(io)
        getattr(writer, write)(value)
        io.seek(0)
        self.assertEqual(io.read(len(b)), fromJava(b))
        self.assertTrue(len(io.read(1)) == 0)
    ...
    def testVarInt(self):
        def check(i, b):  # type: (int, List[int]) -> None
    ...
    def testDouble(self):
        def check(f, b):  # type: (float, List[int]) -> None
