...
public abstract class WsPerformanceTest extends WsTestBase {
  ...
  protected static final int COUNTER = 1;

  protected static void createSetup(final CountDownLatch latch) {
    PACKET_SERIALIZER = SocketPerformanceTest.PACKET_SERIALIZER;
    SESSION_SETUP_SERVER = PerformanceTest.createSetup(REQUEST_EXECUTOR, null, COUNTER);
    SESSION_SETUP_CLIENT = PerformanceTest.createSetup(REQUEST_EXECUTOR, latch, COUNTER);
  }
